  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :disabled="cartInfoList.length==0" :checked="isAllCheck&&cartInfoList.length>0" @change="updateAllCartChecked">
        <span>全选</span>
      </div>
      <div class="option">
      ...
      <div class="money-box">
        <div class="chosed">已选择
          <span>{{ shopChoose.length }}</span>件商品</div>
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
          <i class="summoney">{{ totalPrice }}</i>
        </div>
        <div class="sumbtn">
          <router-link to="/trade" class="sumbtn">结算</router-link>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改某一个产品的个数 情况>=2，不要想布尔值，可以传这些自定义参数 --- 节流
    ...
    computed: {
      ...
      // 判断底部复选框是否勾选【全部产品都选中】
      isAllCheck() {
        // 遍历数组，只要全部的元素属性都为---返回真，否则返回false
        return this.cartInfoList.every(item=>item.isChecked==1)
      },
      // 已选择x件商品
      shopChoose() {
        // 遍历数组中的isChecked==1的添加到一个新数组
