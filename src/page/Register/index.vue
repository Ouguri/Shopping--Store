<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="password" placeholder="请再次输入你的登录密码" v-model="password1" name="password1" v-validate="{ required: true, is: password }" :class="{ invalid: errors.has('password') }"/>
    ...
    methods: {
      ...
      async getCode() {
        try {
          // 简单判断---至少有数据
          const {phone} = this
          // 如果phone存在，则发送一个action
          phone && await this.$store.dispatch('getCode', phone)
          // 将组建code的属性值变为仓库验证码[验证码直接自己填写]
      ...
      async userRegister() {
        ...
        // 只要有一个表单不成功都不会注册
          if(success) {
            ...
            // 等待成功的结果
            // await (phone && code && password == password1) && this.$store.dispatch('userRegister', {code, phone, password})
            await this.$store.dispatch('userRegister', {code, phone, password})
            // 成功---路由跳转登录
            // alert('注册成功')
